...
export const RightComponentActionButtons = ({handleCallFunction, handleRightDrawer, dropDownMenuValues}: Props) => {
	...
	return (
		<div className={`flex flex-col h-[150px] items-center ${userLogged ? 'justify-around' : 'justify-between'} sm:flex-row sm:mx-0 sm:h-fit ${pathname === '/' ? 'w-full' : 'w-[30vw]'}`}>
			{/* call button */}
			<button type='submit' onClick={handleCallFunction} className='bg-blue-400 flex flex-col items-center w-16 text-white py-2 rounded-2xl sm:w-[8vw] lg:flex-row lg:justify-around lg:max-w-[7vw]'>
				<BiPhone size={20} /> <span className={`hidden sm:inline-block ${userLogged ? 'sm:hidden md:inline-block' : ''} `} >{CALL_US}</span>
			</button>
			{/* login button */}
			{userLogged
